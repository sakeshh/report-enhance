...
from agent.etl_pipeline.io_snippets import resolve_path_python_helper
...
def _emit_step(
    ...
    if action == "trim":
        ...
        ev = (step_meta or {}).get("evidence") if step_meta else None
        fill_strategy = (ev or {}).get("recommended_fill") if isinstance(ev, dict) else None
        ...
        lines.append(f"{ds_var} = {ds_var}.drop(columns=[{c}], errors='ignore')")
    elif action == "nullify_future_dates":
        lines.append(f"{ds_var}[{c}] = pd.to_datetime({ds_var}[{c}], errors='coerce')")
        lines.append(f"{ds_var}.loc[{ds_var}[{c}] > pd.Timestamp.now(tz=None), {c}] = pd.NA")
    elif action == "noop":
        lines.append(f"# Column {c}: no transform (user accepted as-is)")
    else:
...
def generate_python_etl(plan: Dict[str, Any], assessment: Dict[str, Any]) -> str:
    ...
    join_strategy = (
        str(joins[0].get("join_type") or "left")
        if joins
        else "none (single-dataset or transform-only)"
    )
    privacy_steps = [
        str(st.get("privacy_action") or st.get("action"))
        for block in (plan.get("datasets") or {}).values()
        for st in (block or {}).get("steps") or []
        if str(st.get("action") or "") in ("hash_phone", "mask_phone", "exclude_column")
    ]
    privacy_policy = ", ".join(privacy_steps) if privacy_steps else "none"
    row_preservation = "preserve all rows" if never_drop else "subset drops allowed per plan"
    output_format = str(plan.get("engine") or "python")
    ...
    lines: List[str] = [
        ...
        "Policy:",
        f"- Row preservation: {row_preservation}",
    ...
    for ds_name, block in ds_plan.items():
        ...
        for st in steps:
            ...
            for sl in _emit_step(action, st.get("column"), ds_var, step_meta=st):
                lines.append(f"    {sl}")
        ...
        if valid_values_rules:
            ...
            lines.append("")
            if never_drop:
                lines.append("    # Valid-values (never_drop_rows): nullify disallowed values, keep all rows")
                for col, allowed in valid_values_rules.items():
                    c = _col_expr(col)
                    lines.append(f"    if {c} in {ds_var}.columns:")
                    lines.append(
                        f"        _allowed_{_safe_ident(col)} = {{str(v).lower() for v in {repr(list(allowed))}}}"
                    )
                    lines.append(
                        f"        _bad = ~{ds_var}[{c}].astype(str).str.lower().isin(_allowed_{_safe_ident(col)}) & {ds_var}[{c}].notna()"
                    )
                    lines.append(f"        if _bad.any():")
                    lines.append(
                        f"            logging.warning(f'Setting invalid {{_bad.sum()}} values to NA on {col} in {ds_name}')"
                    )
                    lines.append(f"        {ds_var}.loc[_bad, {c}] = pd.NA")
            else:
                ...
                for col, allowed in valid_values_rules.items():
                    ...
                    lines.append(
                        f"        {ds_var} = {ds_var}[{ds_var}[{c}].astype(str).str.lower().isin(_allowed_{_safe_ident(col)}) | {ds_var}[{c}].isna()]"
                    )
                    lines.append(f"        _dropped = _before - len({ds_var})")
                    lines.append(f"        if _dropped > 0:")
                    lines.append(
                        f"            logging.info(f'Dropped {{_dropped}} rows in {ds_name} due to valid_values rule on {col}')"
                    )
    ...
    if rel.get("load_order"):
        lines.append(f"# Recommended load/clean order: {rel.get('load_order')}")
        lines.append("")
    if rel.get("many_to_many"):
        lines.append("# M:N relationships — bridge tables emitted in build_bridge_tables()")
        for b in rel["many_to_many"]:
            lines.append(
                f"#   {b.get('dataset_a')}.{b.get('column_a')} <-> "
                f"{b.get('dataset_b')}.{b.get('column_b')} → {b.get('bridge_name')}"
            )
        lines.append("")
